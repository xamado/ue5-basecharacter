...
CSV_DEFINE_CATEGORY(BaseCharacterMovement, true);

DEFINE_LOG_CATEGORY_STATIC(LogBaseCharacterMovement, Log, All);
...
const FVector UBaseCharacterMovementComponent::DefaultGravityDirection = FVector(0.0, 0.0, -1.0);
const float UBaseCharacterMovementComponent::MIN_TICK_TIME = 1e-6f;
const float UBaseCharacterMovementComponent::MIN_FLOOR_DIST = 1.9f;
const float UBaseCharacterMovementComponent::MAX_FLOOR_DIST = 2.4f;
const float UBaseCharacterMovementComponent::BRAKE_TO_STOP_VELOCITY = 10.f;
const float UBaseCharacterMovementComponent::SWEEP_EDGE_REJECT_DISTANCE = 0.15f;
...
void FBaseFindFloorResult::SetFromSweep(const FHitResult& InHit, const float InSweepFloorDist, const bool bIsWalkableFloor)
{
...
void FBaseFindFloorResult::SetFromLineTrace(const FHitResult& InHit, const float InSweepFloorDist, const float InLineDist, const bool bIsWalkableFloor)
{
...
void FBaseCharacterMovementComponentPostPhysicsTickFunction::ExecuteTick(float DeltaTime, enum ELevelTick TickType, ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
...
FString FBaseCharacterMovementComponentPostPhysicsTickFunction::DiagnosticMessage()
{
	return Target->GetFullName() + TEXT("[UBaseCharacterMovementComponent::PostPhysicsTick]");
}

FName FBaseCharacterMovementComponentPostPhysicsTickFunction::DiagnosticContext(bool bDetailed)
{
	if (bDetailed)
	{
...
void FBaseCharacterMovementComponentPrePhysicsTickFunction::ExecuteTick(float DeltaTime, ELevelTick TickType, ENamedThreads::Type CurrentThread, const FGraphEventRef& MyCompletionGraphEvent)
{
	FActorComponentTickFunction::ExecuteTickHelper(Target, /*bTickInEditor=*/ false, DeltaTime, TickType, [this](float DilatedTime)
		{
...
FString FBaseCharacterMovementComponentPrePhysicsTickFunction::DiagnosticMessage()
{
	return Target->GetFullName() + TEXT("[UBaseCharacterMovementComponent::PrePhysicsTick]");
}

FName FBaseCharacterMovementComponentPrePhysicsTickFunction::DiagnosticContext(bool bDetailed)
{
	if (bDetailed)
	{
...
void UBaseCharacterMovementComponent::PostLoad()
{
	...
	if (LinkerUEVer < VER_UE4_CHARACTER_MOVEMENT_WALKABLE_FLOOR_REFACTOR)
	{
		...
		UBaseCharacterMovementComponent::SetWalkableFloorZ(WalkableFloorZ);
	}
...
#if WITH_EDITOR
void UBaseCharacterMovementComponent::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	Super::PostEditChangeProperty(PropertyChangedEvent);

	const FProperty* PropertyThatChanged = PropertyChangedEvent.MemberProperty;
	if (PropertyThatChanged && PropertyThatChanged->GetFName() == GET_MEMBER_NAME_CHECKED(UBaseCharacterMovementComponent, WalkableFloorAngle))
	{
...
void UBaseCharacterMovementComponent::BeginDestroy()
{
...
void UBaseCharacterMovementComponent::Deactivate()
{
...
void UBaseCharacterMovementComponent::SetUpdatedComponent(USceneComponent* NewUpdatedComponent)
{
	if (NewUpdatedComponent)
	{
		const ABaseCharacter* NewCharacterOwner = Cast<ABaseCharacter>(NewUpdatedComponent->GetOwner());
		if (NewCharacterOwner == nullptr)
		{
			UE_LOG(LogBaseCharacterMovement, Error, TEXT("%s owned by %s must update a component owned by a Character"), *GetName(), *GetNameSafe(NewUpdatedComponent->GetOwner()));
			return;
		}
		...
		{
			UE_LOG(LogBaseCharacterMovement, Error, TEXT("%s owned by %s must update a capsule component"), *GetName(), *GetNameSafe(NewUpdatedComponent->GetOwner()));
			return;
		}
	}
	...
	{
		OldPrimitive->OnComponentBeginOverlap.RemoveDynamic(this, &UBaseCharacterMovementComponent::CapsuleTouched);
	}
	...
	{
		UpdatedPrimitive->OnComponentBeginOverlap.AddUniqueDynamic(this, &UBaseCharacterMovementComponent::CapsuleTouched);
	}
...
bool UBaseCharacterMovementComponent::HasValidData() const
{
	...
	if (bIsValid)
	{
		...
		{
			logOrEnsureNanError(TEXT("UBaseCharacterMovementComponent::HasValidData() detected NaN/INF for (%s) in Velocity:\n%s"), *GetPathNameSafe(this), *Velocity.ToString());
		...
		{
			logOrEnsureNanError(TEXT("UBaseCharacterMovementComponent::HasValidData() detected NaN/INF for (%s) in UpdatedComponent->ComponentTransform:\n%s"), *GetPathNameSafe(this), *UpdatedComponent->GetComponentTransform().ToHumanReadableString());
		}
		if (UpdatedComponent->GetComponentRotation().ContainsNaN())
		{
			logOrEnsureNanError(TEXT("UBaseCharacterMovementComponent::HasValidData() detected NaN/INF for (%s) in UpdatedComponent->GetComponentRotation():\n%s"), *GetPathNameSafe(this), *UpdatedComponent->GetComponentRotation().ToString());
		}
	}
#endif
	return bIsValid;
...
bool UBaseCharacterMovementComponent::ShouldUsePackedMovementRPCs() const
{
	return BaseCharacterMovementCVars::NetUsePackedMovementRPCs != 0;
}

FCollisionShape UBaseCharacterMovementComponent::GetPawnCapsuleCollisionShape(const EBaseShrinkCapsuleExtent ShrinkMode, const float CustomShrinkAmount) const
{
	FVector Extent = GetPawnCapsuleExtent(ShrinkMode, CustomShrinkAmount);
	return FCollisionShape::MakeCapsule(Extent);
}

FVector UBaseCharacterMovementComponent::GetPawnCapsuleExtent(const EBaseShrinkCapsuleExtent ShrinkMode, const float CustomShrinkAmount) const
{
	...
	case CGSHRINK_RadiusCustom:
		RadiusEpsilon = CustomShrinkAmount;
		break;

	case CGSHRINK_HeightCustom:
		HeightEpsilon = CustomShrinkAmount;
		break;
		
	case CGSHRINK_AllCustom:
		RadiusEpsilon = CustomShrinkAmount;
		HeightEpsilon = CustomShrinkAmount;
		break;

	default:
		UE_LOG(LogBaseCharacterMovement, Warning, TEXT("Unknown EShrinkCapsuleExtent in UBaseCharacterMovementComponent::GetCapsuleExtent"));
		break;
	}
...
FVector UBaseCharacterMovementComponent::GetImpartedMovementBaseVelocity() const
{
...
void UBaseCharacterMovementComponent::Launch(FVector const& LaunchVel)
{
...
bool UBaseCharacterMovementComponent::HandlePendingLaunch()
{
...
void UBaseCharacterMovementComponent::JumpOff(AActor* MovementBaseActor)
{
...
FVector UBaseCharacterMovementComponent::GetBestDirectionOffActor(AActor* BaseActor) const
{
...
float UBaseCharacterMovementComponent::GetNetworkSafeRandomAngleDegrees() const
{
...
void UBaseCharacterMovementComponent::SetDefaultMovementMode()
{
...
void UBaseCharacterMovementComponent::SetGroundMovementMode(EMovementMode NewGroundMovementMode)
{
...
void UBaseCharacterMovementComponent::OnMovementModeChanged(EMovementMode PreviousMovementMode, uint8 PreviousCustomMode)
{
	...
	else
	{
		...
		if (MovementMode == MOVE_Falling)
		{
			...
			if (!BaseCharacterMovementCVars::bAddFormerBaseVelocityToRootMotionOverrideWhenFalling || FormerBaseVelocityDecayHalfLife == 0.f)
			{
...
uint8 UBaseCharacterMovementComponent::PackNetworkMovementMode() const
{
...
void UBaseCharacterMovementComponent::ApplyNetworkMovementMode(const uint8 ReceivedMode)
{
...
void UBaseCharacterMovementComponent::Serialize(FArchive& Archive)
{
...
void UBaseCharacterMovementComponent::AddReferencedObjects(UObject* InThis, FReferenceCollector& Collector)
{
	Super::AddReferencedObjects(InThis, Collector);

	UBaseCharacterMovementComponent* This = CastChecked<UBaseCharacterMovementComponent>(InThis);
...
{
	// SCOPED_NAMED_EVENT(UBaseCharacterMovementComponent_TickComponent, FColor::Yellow);
	...
	if (CharacterOwner->GetLocalRole() > ROLE_SimulatedProxy)
	{
		...
		else if (CharacterOwner->GetRemoteRole() == ROLE_AutonomousProxy)
		{
			...
			if (BaseCharacterMovementCVars::NetEnableListenServerSmoothing && !bNetworkSmoothingComplete && IsNetMode(NM_ListenServer))
			{
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
	const bool bVisualizeMovement = BaseCharacterMovementCVars::VisualizeMovement > 0;
...
void UBaseCharacterMovementComponent::AdjustProxyCapsuleSize()
{
	if (bShrinkProxyCapsule && CharacterOwner && CharacterOwner->GetLocalRole() == ROLE_SimulatedProxy)
	{
		...
		{
			UE_LOG(LogBaseCharacterMovement, Warning, TEXT("Invalid attempt to shrink Proxy capsule for %s to zero dimension!"), *CharacterOwner->GetName());
			return;
		}

		UE_LOG(LogBaseCharacterMovement, Verbose, TEXT("Shrinking capsule for %s from (r=%.3f, h=%.3f) to (r=%.3f, h=%.3f)"), *CharacterOwner->GetName(),
...
void UBaseCharacterMovementComponent::SimulatedTick(float DeltaSeconds)
{
	...
	if (CharacterOwner->IsPlayingNetworkedRootMotionMontage())
	{
		...
		UE_LOG(LogRootMotion, Verbose, TEXT("UBaseCharacterMovementComponent::SimulatedTick"));
		...
		if( RootMotionParams.bHasRootMotion )
		{
			...
			if (CharacterOwner && false)
			{
				...
				UE_LOG(LogRootMotion, Log,  TEXT("UBaseCharacterMovementComponent::SimulatedTick DeltaMovement Translation: %s, Rotation: %s, MovementBase: %s"),
	...
	else if (CurrentRootMotion.HasActiveRootMotionSources())
	{
		...
		if( CharacterOwner->RootMotionRepMoves.Num() > 0 )
		{
			...
			FBaseSimulatedRootMotionReplicatedMove& RootMotionRepMove = CharacterOwner->RootMotionRepMoves.Last();
...
FTransform UBaseCharacterMovementComponent::ConvertLocalRootMotionToWorld(const FTransform& LocalRootMotionTransform, float DeltaSeconds)
{
...
void UBaseCharacterMovementComponent::SimulateRootMotion(float DeltaSeconds, const FTransform& LocalRootMotionTransform)
{
...
FVector UBaseCharacterMovementComponent::CalcAnimRootMotionVelocity(const FVector& RootMotionDeltaMove, float DeltaSeconds, const FVector& CurrentVelocity) const
{
...
FVector UBaseCharacterMovementComponent::ConstrainAnimRootMotionVelocity(const FVector& RootMotionVelocity, const FVector& CurrentVelocity) const
{
...
void UBaseCharacterMovementComponent::SimulateMovement(float DeltaSeconds)
{
	...
	{
		UE_LOG(LogBaseCharacterMovement, Verbose, TEXT("Base for simulated character '%s' is not resolved on client, skipping SimulateMovement"), *CharacterOwner->GetName());
		return;
	}
	...
	// Scoped updates can improve performance of multiple MoveComponent calls.
	{
		...
		if (bIsSimulatedProxy)
		{
			...
			if (bNetworkUpdateReceived)
			{
				...
				UE_LOG(LogBaseCharacterMovement, Verbose, TEXT("Proxy %s received net update"), *CharacterOwner->GetName());
				if (bNetworkGravityDirectionChanged)
				{
					SetGravityDirection(CharacterOwner->GetReplicatedGravityDirection());
					bNetworkGravityDirectionChanged = false;
		...
		if (!bHandledNetUpdate || !bNetworkSkipProxyPredictionOnNetUpdate || !BaseCharacterMovementCVars::NetEnableSkipProxyPredictionOnNetUpdate)
		{
			UE_LOG(LogBaseCharacterMovement, Verbose, TEXT("Proxy %s simulating movement"), *GetNameSafe(CharacterOwner));
			FBaseStepDownResult StepDownResult;
		...
		{
			UE_LOG(LogBaseCharacterMovement, Verbose, TEXT("Proxy %s SKIPPING simulate movement"), *GetNameSafe(CharacterOwner));
		}
	...
	if (BaseCharacterMovementCVars::BasedMovementMode == 0)
	{
...
UPrimitiveComponent* UBaseCharacterMovementComponent::GetMovementBase() const
{
	return CharacterOwner ? CharacterOwner->GetMovementBase() : NULL;
}

void UBaseCharacterMovementComponent::SetBase( UPrimitiveComponent* NewBase, FName BoneName, bool bNotifyActor )
{
...
void UBaseCharacterMovementComponent::SetBaseFromFloor(const FBaseFindFloorResult& FloorResult)
{
...
void UBaseCharacterMovementComponent::MaybeUpdateBasedMovement(float DeltaSeconds)
{
	bDeferUpdateBasedMovement = false;
	...
	if (MovementBaseUtility::UseRelativeLocation(MovementBase))
	{
		...
		else
		{
			...
			if (BaseCharacterMovementCVars::BasedMovementMode == 2)
			{
				UpdateBasedMovement(DeltaSeconds);
			}
		}
...
void UBaseCharacterMovementComponent::MaybeSaveBaseLocation()
{
	if (BaseCharacterMovementCVars::BasedMovementMode == 2 || !bDeferUpdateBasedMovement)
	{
		SaveBaseLocation();
	}
}
...
void UBaseCharacterMovementComponent::UpdateBasedMovement(float DeltaSeconds)
{
	if (!HasValidData())
	{
		return;
...
void UBaseCharacterMovementComponent::OnUnableToFollowBaseMove(const FVector& DeltaPosition, const FVector& OldLocation, const FHitResult& MoveOnBaseHit)
{
	// no default implementation, left for subclasses to override.
}


void UBaseCharacterMovementComponent::UpdateBasedRotation(FRotator& FinalRotation, const FRotator& ReducedRotation)
{
...
void UBaseCharacterMovementComponent::DisableMovement()
{
	if (CharacterOwner)
	{
...
void UBaseCharacterMovementComponent::PerformMovement(float DeltaSeconds)
{
	...
	if( CurrentRootMotion.HasAdditiveVelocity() )
	{
		...
		if (BaseRootMotionSourceDebug::CVarDebugRootMotionSources.GetValueOnGameThread() == 1)
		{
			if (!Adjustment.IsNearlyZero())
			{
				...
				BaseRootMotionSourceDebug::PrintOnScreen(*CharacterOwner, AdjustedDebugString);
			}
		}
#endif
	}
	...
	if (bHasAuthority && UpdatedComponent && !IsNetMode(NM_Client))
	{
		...
		if (bLocationChanged || bRotationChanged)
		{
			...
			if (bIsRemotePlayer && ServerData && BaseCharacterMovementCVars::NetUseClientTimestampForReplicatedTransform)
			{
...
bool UBaseCharacterMovementComponent::ShouldCancelAdaptiveReplication() const
{
...
void UBaseCharacterMovementComponent::CallMovementUpdateDelegate(float DeltaTime, const FVector& OldLocation, const FVector& OldVelocity)
{
...
void UBaseCharacterMovementComponent::SaveBaseLocation()
{
	if (!HasValidData())
	{
		return;
...
void UBaseCharacterMovementComponent::SetCrouchedHalfHeight(const float NewValue)
{
...
float UBaseCharacterMovementComponent::GetCrouchedHalfHeight() const
{ 
	return CrouchedHalfHeight; 
}

void UBaseCharacterMovementComponent::Crouch(bool bClientSimulation)
{
	if (!HasValidData())
	{
		return;
	...
	if( !bClientSimulation )
	{
		...
		if (ClampedCrouchedHalfHeight > OldUnscaledHalfHeight)
		{
			...
			const bool bEncroached = GetWorld()->OverlapBlockingTestByChannel(UpdatedComponent->GetComponentLocation() - FVector(0.f,0.f,ScaledHalfHeightAdjust), FQuat::Identity,
				UpdatedComponent->GetCollisionObjectType(), GetPawnCapsuleCollisionShape(CGSHRINK_None), CapsuleParams, ResponseParam);
...
void UBaseCharacterMovementComponent::UnCrouch(bool bClientSimulation)
{
	if (!HasValidData())
	{
		return;
	...
	if( !bClientSimulation )
	{
		...
		const FCollisionShape StandingCapsuleShape = GetPawnCapsuleCollisionShape(CGSHRINK_HeightCustom, -SweepInflation - ScaledHalfHeightAdjust); // Shrink by negative amount, so actually grow it.
		...
		if (!bCrouchMaintainsBaseLocation)
		{
			...
			if (bEncroached)
			{
				...
				if (ScaledHalfHeightAdjust > 0.f)
				{
					...
					FHitResult Hit(1.f);
					const FCollisionShape ShortCapsuleShape = GetPawnCapsuleCollisionShape(CGSHRINK_HeightCustom, ShrinkHalfHeight);
...
void UBaseCharacterMovementComponent::StartNewPhysics(float deltaTime, int32 Iterations)
{
	...
	{
		UE_LOG(LogBaseCharacterMovement, Log, TEXT("UBaseCharacterMovementComponent::StartNewPhysics: UpdateComponent (%s) is simulating physics - aborting."), *UpdatedComponent->GetPathName());
		return;
	}
...
float UBaseCharacterMovementComponent::GetGravityZ() const
{
...
float UBaseCharacterMovementComponent::GetMinAnalogSpeed() const
{
	switch (MovementMode)
	{
	case MOVE_Walking:
...
FVector UBaseCharacterMovementComponent::GetPenetrationAdjustment(const FHitResult& Hit) const
{
...
bool UBaseCharacterMovementComponent::ResolvePenetrationImpl(const FVector& Adjustment, const FHitResult& Hit, const FQuat& NewRotation)
{
...
float UBaseCharacterMovementComponent::SlideAlongSurface(const FVector& Delta, float Time, const FVector& InNormal, FHitResult& Hit, bool bHandleImpact)
{
	if (!Hit.bBlockingHit)
	{
		return 0.f;
...
void UBaseCharacterMovementComponent::TwoWallAdjust(FVector& WorldSpaceDelta, const FHitResult& Hit, const FVector& OldHitNormal) const
{
...
FVector UBaseCharacterMovementComponent::ComputeSlideVector(const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const
{
...
FVector UBaseCharacterMovementComponent::HandleSlopeBoosting(const FVector& SlideResult, const FVector& Delta, const float Time, const FVector& Normal, const FHitResult& Hit) const
{
...
FVector UBaseCharacterMovementComponent::NewFallVelocity(const FVector& InitialVelocity, const FVector& Gravity, float DeltaTime) const
{
...
float UBaseCharacterMovementComponent::ImmersionDepth() const
{
	float depth = 0.f;
...
bool UBaseCharacterMovementComponent::IsFlying() const
{
...
bool UBaseCharacterMovementComponent::IsFalling() const
{
	return (MovementMode == MOVE_Falling) && UpdatedComponent;
}

bool UBaseCharacterMovementComponent::IsSwimming() const
{
	return (MovementMode == MOVE_Swimming) && UpdatedComponent;
}

bool UBaseCharacterMovementComponent::IsCrouching() const
{
...
bool UBaseCharacterMovementComponent::ShouldComputeAccelerationToReachRequestedVelocity(const float RequestedSpeed) const
{
...
bool UBaseCharacterMovementComponent::ApplyRequestedMove(float DeltaTime, float MaxAccel, float MaxSpeed, float Friction, float BrakingDeceleration, FVector& OutAcceleration, float& OutRequestedSpeed)
{
...
void UBaseCharacterMovementComponent::RequestDirectMove(const FVector& MoveVelocity, bool bForceMaxSpeed)
{
...
bool UBaseCharacterMovementComponent::ShouldPerformAirControlForPathFollowing() const
{
	return IsFalling();
}

void UBaseCharacterMovementComponent::RequestPathMove(const FVector& MoveInput)
{
...
bool UBaseCharacterMovementComponent::CanStartPathFollowing() const
{
...
bool UBaseCharacterMovementComponent::CanStopPathFollowing() const
{
	return !IsFalling();
}

float UBaseCharacterMovementComponent::GetPathFollowingBrakingDistance(float MaxSpeed) const
{
...
void UBaseCharacterMovementComponent::CalcAvoidanceVelocity(float DeltaTime)
{
...
void UBaseCharacterMovementComponent::PostProcessAvoidanceVelocity(FVector& NewVelocity)
{
	// empty in base class
}

void UBaseCharacterMovementComponent::UpdateDefaultAvoidance()
{
	if (!bUseRVOAvoidance)
	{
		return;
...
void UBaseCharacterMovementComponent::SetRVOAvoidanceUID(int32 UID)
{
	AvoidanceUID = UID;
}

int32 UBaseCharacterMovementComponent::GetRVOAvoidanceUID()
{
	return AvoidanceUID;
}

void UBaseCharacterMovementComponent::SetRVOAvoidanceWeight(float Weight)
{
	AvoidanceWeight = Weight;
}

float UBaseCharacterMovementComponent::GetRVOAvoidanceWeight()
{
	return AvoidanceWeight;
}

FVector UBaseCharacterMovementComponent::GetRVOAvoidanceOrigin()
{
	return GetActorFeetLocation();
}

float UBaseCharacterMovementComponent::GetRVOAvoidanceRadius()
{
...
float UBaseCharacterMovementComponent::GetRVOAvoidanceConsiderationRadius()
{
	return AvoidanceConsiderationRadius;
}

float UBaseCharacterMovementComponent::GetRVOAvoidanceHeight()
{
	UCapsuleComponent* CapsuleComp = GetCharacterOwner()->GetCapsuleComponent();
	return CapsuleComp ? CapsuleComp->GetScaledCapsuleHalfHeight() : 0.0f;
}

FVector UBaseCharacterMovementComponent::GetVelocityForRVOConsideration()
{
	return Velocity;
}

void UBaseCharacterMovementComponent::SetAvoidanceGroupMask(int32 GroupFlags)
{
	AvoidanceGroup.SetFlagsDirectly(GroupFlags);
}

int32 UBaseCharacterMovementComponent::GetAvoidanceGroupMask()
{
	return AvoidanceGroup.Packed;
}

void UBaseCharacterMovementComponent::SetGroupsToAvoidMask(int32 GroupFlags)
{
	GroupsToAvoid.SetFlagsDirectly(GroupFlags);
}

int32 UBaseCharacterMovementComponent::GetGroupsToAvoidMask()
{
	return GroupsToAvoid.Packed;
}

void UBaseCharacterMovementComponent::SetGroupsToIgnoreMask(int32 GroupFlags)
{
	GroupsToIgnore.SetFlagsDirectly(GroupFlags);
}

int32 UBaseCharacterMovementComponent::GetGroupsToIgnoreMask()
{
	return GroupsToIgnore.Packed;
}

void UBaseCharacterMovementComponent::SetAvoidanceVelocityLock(class UAvoidanceManager* Avoidance, float Duration)
{
...
void UBaseCharacterMovementComponent::NotifyBumpedPawn(APawn* BumpedPawn)
{
...
float UBaseCharacterMovementComponent::GetMaxJumpHeight() const
{
...
float UBaseCharacterMovementComponent::GetMaxJumpHeightWithJumpTime() const
{
...
FVector UBaseCharacterMovementComponent::GetCurrentAcceleration() const
{
	return Acceleration;
}

void UBaseCharacterMovementComponent::ApplyVelocityBraking(float DeltaTime, float Friction, float BrakingDeceleration)
{
...
void UBaseCharacterMovementComponent::PhysFlying(float deltaTime, int32 Iterations)
{
...
void UBaseCharacterMovementComponent::RestorePreAdditiveRootMotionVelocity()
{
...
void UBaseCharacterMovementComponent::ApplyRootMotionToVelocity(float deltaTime)
{
	SCOPE_CYCLE_COUNTER(STAT_CharacterMovementRootMotionSourceApply);
	...
	{
		float LiftoffBound;
		if( CurrentRootMotion.LastAccumulatedSettings.HasFlag(EBaseRootMotionSourceSettingsFlags::UseSensitiveLiftoffCheck) )
		{
...
void UBaseCharacterMovementComponent::DecayFormerBaseVelocity(float deltaTime)
{
	if (!BaseCharacterMovementCVars::bAddFormerBaseVelocityToRootMotionOverrideWhenFalling || FormerBaseVelocityDecayHalfLife == 0.f)
	{
		DecayingFormerBaseVelocity = FVector::ZeroVector;
...
void UBaseCharacterMovementComponent::HandleSwimmingWallHit(const FHitResult& Hit, float DeltaTime)
{
}

void UBaseCharacterMovementComponent::PhysSwimming(float deltaTime, int32 Iterations)
{
	if (deltaTime < MIN_TICK_TIME)
	{
		return;
...
void UBaseCharacterMovementComponent::StartSwimming(FVector OldLocation, FVector OldVelocity, float timeTick, float remainingTime, int32 Iterations)
{
...
float UBaseCharacterMovementComponent::Swim(FVector Delta, FHitResult& Hit)
{
...
FVector UBaseCharacterMovementComponent::FindWaterLine(FVector InWater, FVector OutofWater)
{
...
void UBaseCharacterMovementComponent::NotifyJumpApex() 
{
	if( CharacterOwner )
	{
...
FVector UBaseCharacterMovementComponent::GetFallingLateralAcceleration(float DeltaTime)
{
...
bool UBaseCharacterMovementComponent::ShouldLimitAirControl(float DeltaTime, const FVector& FallAcceleration) const
{
...
void UBaseCharacterMovementComponent::SetGravityDirection(const FVector& InNewGravityDir)
{
	...
	if (ensure(!NewGravityDir.IsNearlyZero()))
	{
		...
		{
			UE_LOG(LogBaseCharacterMovement, Verbose, TEXT("SetGravityDirection: From(%s) To(%s)"), *GravityDirection.ToCompactString(), *NewGravityDir.ToCompactString());
...
float UBaseCharacterMovementComponent::BoostAirControl(float DeltaTime, float TickAirControl, const FVector& FallAcceleration)
{
...
void UBaseCharacterMovementComponent::PhysFalling(float deltaTime, int32 Iterations)
{
	...
	while( (remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) )
	{
		...
		//UE_LOG(LogBaseCharacterMovement, Log, TEXT("dt=(%.6f) OldLocation=(%s) OldVelocity=(%s) OldVelocityWithRootMotion=(%s) NewVelocity=(%s)"), timeTick, *(UpdatedComponent->GetComponentLocation()).ToString(), *OldVelocity.ToString(), *OldVelocityWithRootMotion.ToString(), *Velocity.ToString());
		...
		if (BaseCharacterMovementCVars::ForceJumpPeakSubstep && GravityRelativeOldVelocityWithRootMotion.Z > 0.f && RotateWorldToGravity(Velocity).Z <= 0.f && NumJumpApexAttempts < MaxJumpApexAttemptsPerSimulation)
		{
			...
			if (!FMath::IsNearlyZero(GravityRelativeDerivedAccel.Z))
			{
				...
				if (TimeToApex >= ApexTimeMinimum && TimeToApex < timeTick)
				{
					...
					for (TSharedPtr<FBaseRootMotionSource> RootMotionSource : CurrentRootMotion.RootMotionSources)
					{
		...
		else if ( Hit.bBlockingHit )
		{
			...
			else
			{
				...
				{
					const FVector PawnLocation = UpdatedComponent->GetComponentLocation();
					FBaseFindFloorResult FloorResult;
				...
				if (BaseCharacterMovementCVars::UseTargetVelocityOnImpact && !Velocity.IsNearlyZero() && MovementBaseUtility::IsSimulatedBase(HitComponent))
				{
...
FVector UBaseCharacterMovementComponent::LimitAirControl(float DeltaTime, const FVector& FallAcceleration, const FHitResult& HitResult, bool bCheckForValidLandingSpot)
{
...
bool UBaseCharacterMovementComponent::CheckLedgeDirection(const FVector& OldLocation, const FVector& SideStep, const FVector& GravDir) const
{
	...
	FCollisionResponseParams ResponseParam;
	InitCollisionParams(CapsuleParams, ResponseParam);
	const FCollisionShape CapsuleShape = GetPawnCapsuleCollisionShape(CGSHRINK_None);
	const ECollisionChannel CollisionChannel = UpdatedComponent->GetCollisionObjectType();
...
FVector UBaseCharacterMovementComponent::GetLedgeMove(const FVector& OldLocation, const FVector& Delta, const FVector& GravDir) const
{
...
bool UBaseCharacterMovementComponent::CanWalkOffLedges() const
{
...
bool UBaseCharacterMovementComponent::CheckFall(const FBaseFindFloorResult& OldFloor, const FHitResult& Hit, const FVector& Delta, const FVector& OldLocation, float remainingTime, float timeTick, int32 Iterations, bool bMustJump)
{
	if (!HasValidData())
	{
		return false;
...
void UBaseCharacterMovementComponent::StartFalling(int32 Iterations, float remainingTime, float timeTick, const FVector& Delta, const FVector& subLoc)
{
	// start falling 
	const float DesiredDist = Delta.Size();
...
void UBaseCharacterMovementComponent::RevertMove(const FVector& OldLocation, UPrimitiveComponent* OldBase, const FVector& PreviousBaseLocation, const FBaseFindFloorResult& OldFloor, bool bFailMove)
{
	//UE_LOG(LogBaseCharacterMovement, Log, TEXT("RevertMove from %f %f %f to %f %f %f"), CharacterOwner->Location.X, CharacterOwner->Location.Y, CharacterOwner->Location.Z, OldLocation.X, OldLocation.Y, OldLocation.Z);
	UpdatedComponent->SetWorldLocation(OldLocation, false, nullptr, GetTeleportType());
	
	//UE_LOG(LogBaseCharacterMovement, Log, TEXT("Now at %f %f %f"), CharacterOwner->Location.X, CharacterOwner->Location.Y, CharacterOwner->Location.Z);
	bJustTeleported = false;
	...
	if ( bFailMove )
	{
		...
		Velocity = FVector::ZeroVector;
		Acceleration = FVector::ZeroVector;
		//UE_LOG(LogBaseCharacterMovement, Log, TEXT("%s FAILMOVE RevertMove"), *CharacterOwner->GetName());
	}
}


FVector UBaseCharacterMovementComponent::ComputeGroundMovementDelta(const FVector& Delta, const FHitResult& RampHit, const bool bHitFromLineTrace) const
{
	if (!HasCustomGravity())
	{
...
void UBaseCharacterMovementComponent::OnCharacterStuckInGeometry(const FHitResult* Hit)
{
	if (BaseCharacterMovementCVars::StuckWarningPeriod >= 0)
	{
		const UWorld* MyWorld = GetWorld();
		const float RealTimeSeconds = MyWorld->GetRealTimeSeconds();
		if ((RealTimeSeconds - LastStuckWarningTime) >= BaseCharacterMovementCVars::StuckWarningPeriod)
		{
			LastStuckWarningTime = RealTimeSeconds;
			if (Hit == nullptr)
			{
				UE_LOG(LogBaseCharacterMovement, Log, TEXT("%s is stuck and failed to move! (%d other events since notify)"), *CharacterOwner->GetName(), StuckWarningCountSinceNotify);
			}
			else
			{
				UE_LOG(LogBaseCharacterMovement, Log, TEXT("%s is stuck and failed to move! Velocity: X=%3.2f Y=%3.2f Z=%3.2f Location: X=%3.2f Y=%3.2f Z=%3.2f Normal: X=%3.2f Y=%3.2f Z=%3.2f PenetrationDepth:%.3f Actor:%s Component:%s BoneName:%s (%d other events since notify)"),
...
void UBaseCharacterMovementComponent::MoveAlongFloor(const FVector& InVelocity, float DeltaSeconds, FBaseStepDownResult* OutStepDownResult)
{
	...
	else if (Hit.IsValidBlockingHit())
	{
		...
		if (Hit.IsValidBlockingHit())
		{
			if (CanStepUp(Hit) || (CharacterOwner->GetMovementBase() != nullptr && Hit.HitObjectHandle == CharacterOwner->GetMovementBase()->GetOwner()))
			{
				...
				{
					UE_LOG(LogBaseCharacterMovement, Verbose, TEXT("- StepUp (ImpactNormal %s, Normal %s"), *Hit.ImpactNormal.ToString(), *Hit.Normal.ToString());
...
void UBaseCharacterMovementComponent::MaintainHorizontalGroundVelocity()
{
	FVector GravityRelativeVelocity = RotateWorldToGravity(Velocity);
...
void UBaseCharacterMovementComponent::PhysWalking(float deltaTime, int32 Iterations)
{
	...
	while ( (remainingTime >= MIN_TICK_TIME) && (Iterations < MaxSimulationIterations) && CharacterOwner && (CharacterOwner->Controller || bRunPhysicsWithNoController || HasAnimRootMotion() || CurrentRootMotion.HasOverrideVelocity() || (CharacterOwner->GetLocalRole() == ROLE_SimulatedProxy)) )
	{
		...
		const FVector OldLocation = UpdatedComponent->GetComponentLocation();
		const FBaseFindFloorResult OldFloor = CurrentFloor;
...
void UBaseCharacterMovementComponent::PhysNavWalking(float deltaTime, int32 Iterations)
{
...
bool UBaseCharacterMovementComponent::FindNavFloor(const FVector& TestLocation, FNavLocation& NavFloorLocation) const
{
...
FVector UBaseCharacterMovementComponent::ProjectLocationFromNavMesh(float DeltaSeconds, const FVector& CurrentFeetLocation, const FVector& TargetNavLocation, float UpOffset, float DownOffset)
{
	SCOPE_CYCLE_COUNTER(STAT_CharNavProjectLocation);
...
void UBaseCharacterMovementComponent::FindBestNavMeshLocation(const FVector& TraceStart, const FVector& TraceEnd, const FVector& CurrentFeetLocation, const FVector& TargetNavLocation, FHitResult& OutHitResult) const
{
...
const INavigationDataInterface* UBaseCharacterMovementComponent::GetNavData() const
{
...
void UBaseCharacterMovementComponent::HandleWalkingOffLedge(const FVector& PreviousFloorImpactNormal, const FVector& PreviousFloorContactNormal, const FVector& PreviousLocation, float TimeDelta)
{
	if (CharacterOwner)
	{
...
void UBaseCharacterMovementComponent::AdjustFloorHeight()
{
	...
	if (CurrentFloor.bLineTrace)
	{
		if (OldFloorDist < MIN_FLOOR_DIST && CurrentFloor.LineDist >= MIN_FLOOR_DIST)
		{
			...
			UE_LOG(LogBaseCharacterMovement, VeryVerbose, TEXT("Adjust floor height aborting due to line trace with small floor distance (line: %.2f, sweep: %.2f)"), CurrentFloor.LineDist, CurrentFloor.FloorDist);
			return;
		}
		else
		{
	...
	if (OldFloorDist < MIN_FLOOR_DIST || OldFloorDist > MAX_FLOOR_DIST)
	{
		...
		UE_LOG(LogBaseCharacterMovement, VeryVerbose, TEXT("Adjust floor height %.3f (Hit = %d)"), MoveDist, AdjustHit.bBlockingHit);
...
void UBaseCharacterMovementComponent::StopActiveMovement() 
{ 
...
void UBaseCharacterMovementComponent::ProcessLanded(const FHitResult& Hit, float remainingTime, int32 Iterations)
{
...
void UBaseCharacterMovementComponent::ControlledCharacterMove(const FVector& InputVector, float DeltaSeconds)
{
	{
...
void UBaseCharacterMovementComponent::SetNavWalkingPhysics(bool bEnable)
{
	if (UpdatedPrimitive)
	{
		if (bEnable)
		...
		else
		{
			...
			{
				UE_LOG(LogBaseCharacterMovement, Warning, TEXT("Can't revert NavWalking collision settings for %s.%s"),
...
bool UBaseCharacterMovementComponent::TryToLeaveNavWalking()
{
	SetNavWalkingPhysics(false);
...
void UBaseCharacterMovementComponent::OnTeleported()
{
	if (!HasValidData())
	{
		return;
...
FRotator UBaseCharacterMovementComponent::ComputeOrientToMovementRotation(const FRotator& CurrentRotation, float DeltaTime, FRotator& DeltaRotation) const
{
...
bool UBaseCharacterMovementComponent::ShouldRemainVertical() const
{
...
void UBaseCharacterMovementComponent::PhysicsRotation(float DeltaTime)
{
	...
	if (!CurrentRotation.Equals(DesiredRotation, AngleTolerance))
	{
		...
		if (BaseCharacterMovementCVars::bPreventNonVerticalOrientationBlock && bWantsToBeVertical)
		{
...
void UBaseCharacterMovementComponent::PhysicsVolumeChanged( APhysicsVolume* NewVolume )
{
	if (!HasValidData())
	{
		return;
...
bool UBaseCharacterMovementComponent::ShouldJumpOutOfWater(FVector& JumpDir)
{
...
void UBaseCharacterMovementComponent::JumpOutOfWater(FVector WallNormal) {}


bool UBaseCharacterMovementComponent::CheckWaterJump(FVector CheckPoint, FVector& WallNormal)
{
	if (!HasValidData())
	{
		return false;
	...
	FCollisionResponseParams ResponseParam;
	InitCollisionParams(CapsuleParams, ResponseParam);
	FCollisionShape CapsuleShape = GetPawnCapsuleCollisionShape(CGSHRINK_None);
	const ECollisionChannel CollisionChannel = UpdatedComponent->GetCollisionObjectType();
...
void UBaseCharacterMovementComponent::AddImpulse( FVector Impulse, bool bVelocityChange )
{
	if (!Impulse.IsZero() && (MovementMode != MOVE_None) && IsActive() && HasValidData())
	{
		...
		if ( !bVelocityChange )
		{
			...
			{
				UE_LOG(LogBaseCharacterMovement, Warning, TEXT("Attempt to apply impulse to zero or negative Mass in CharacterMovement"));
			}
		}
...
void UBaseCharacterMovementComponent::AddForce( FVector Force )
{
	if (!Force.IsZero() && (MovementMode != MOVE_None) && IsActive() && HasValidData())
	{
		...
		{
			UE_LOG(LogBaseCharacterMovement, Warning, TEXT("Attempt to apply force to zero or negative Mass in CharacterMovement"));
		}
	}
}

void UBaseCharacterMovementComponent::MoveSmooth(const FVector& InVelocity, const float DeltaSeconds, FBaseStepDownResult* OutStepDownResult)
{
	if (!HasValidData())
	{
		return;
...
void UBaseCharacterMovementComponent::UpdateProxyAcceleration()
{
...
bool UBaseCharacterMovementComponent::IsWalkable(const FHitResult& Hit) const
{
	if (!Hit.IsValidBlockingHit())
	{
...
void UBaseCharacterMovementComponent::SetWalkableFloorAngle(float InWalkableFloorAngle)
{
...
void UBaseCharacterMovementComponent::SetWalkableFloorZ(float InWalkableFloorZ)
{
	WalkableFloorZ = FMath::Clamp(InWalkableFloorZ, 0.f, 1.f);
	WalkableFloorAngle = FMath::RadiansToDegrees(FMath::Acos(WalkableFloorZ));
}

float UBaseCharacterMovementComponent::K2_GetWalkableFloorAngle() const
{
	return GetWalkableFloorAngle();
}

float UBaseCharacterMovementComponent::K2_GetWalkableFloorZ() const
{
...
bool UBaseCharacterMovementComponent::IsWithinEdgeTolerance(const FVector& CapsuleLocation, const FVector& TestImpactPoint, const float CapsuleRadius) const
{
...
void UBaseCharacterMovementComponent::ComputeFloorDist(const FVector& CapsuleLocation, float LineDistance, float SweepDistance, FBaseFindFloorResult& OutFloorResult, float SweepRadius, const FHitResult* DownwardSweepResult) const
{
	UE_LOG(LogBaseCharacterMovement, VeryVerbose, TEXT("[Role:%d] ComputeFloorDist: %s at location %s"), (int32)CharacterOwner->GetLocalRole(), *GetNameSafe(CharacterOwner), *CapsuleLocation.ToString());
...
void UBaseCharacterMovementComponent::FindFloor(const FVector& CapsuleLocation, FBaseFindFloorResult& OutFloorResult, bool bCanUseCachedLocation, const FHitResult* DownwardSweepResult) const
{
	...
	UE_LOG(LogBaseCharacterMovement, VeryVerbose, TEXT("[Role:%d] FindFloor: %s at location %s"), (int32)CharacterOwner->GetLocalRole(), *GetNameSafe(CharacterOwner), *CapsuleLocation.ToString());
	check(CharacterOwner->GetCapsuleComponent());
	...
	if (FloorLineTraceDist > 0.f || FloorSweepTraceDist > 0.f)
	{
		...
		else
		{
			...
			{
				//UE_LOG(LogBaseCharacterMovement, Log, TEXT("%s SKIP check for floor"), *CharacterOwner->GetName());
	...
	if (bNeedToValidateFloor && OutFloorResult.bBlockingHit && !OutFloorResult.bLineTrace)
	{
		...
		if (ShouldComputePerchResult(OutFloorResult.HitResult, bCheckRadius))
		{
			...
			FBaseFindFloorResult PerchFloorResult;
...
void UBaseCharacterMovementComponent::K2_FindFloor(FVector CapsuleLocation, FBaseFindFloorResult& FloorResult) const
{
...
void UBaseCharacterMovementComponent::K2_ComputeFloorDist(FVector CapsuleLocation, float LineDistance, float SweepDistance, float SweepRadius, FBaseFindFloorResult& FloorResult) const
{
	if (HasValidData())
	{
...
bool UBaseCharacterMovementComponent::FloorSweepTest(
	FHitResult& OutHit,
	const FVector& Start,
	const FVector& End,
...
bool UBaseCharacterMovementComponent::IsValidLandingSpot(const FVector& CapsuleLocation, const FHitResult& Hit) const
{
	if (!Hit.bBlockingHit)
	{
		return false;
...
bool UBaseCharacterMovementComponent::ShouldCheckForValidLandingSpot(float DeltaTime, const FVector& Delta, const FHitResult& Hit) const
{
...
float UBaseCharacterMovementComponent::GetPerchRadiusThreshold() const
{
...
float UBaseCharacterMovementComponent::GetValidPerchRadius() const
{
	if (CharacterOwner)
	{
...
bool UBaseCharacterMovementComponent::ShouldComputePerchResult(const FHitResult& InHit, bool bCheckRadius) const
{
...
bool UBaseCharacterMovementComponent::ComputePerchResult(const float TestRadius, const FHitResult& InHit, const float InMaxFloorDist, FBaseFindFloorResult& OutPerchFloorResult) const
{
...
bool UBaseCharacterMovementComponent::CanStepUp(const FHitResult& Hit) const
{
...
bool UBaseCharacterMovementComponent::StepUp(const FVector& GravDir, const FVector& Delta, const FHitResult &InHit, FBaseStepDownResult* OutStepDownResult)
{
	...
	{	
		...
		{
			//UE_LOG(LogBaseCharacterMovement, VeryVerbose, TEXT("- Reject StepUp (too high Height %.3f) up from floor base %f to %f"), DeltaZ, PawnInitialFloorBaseZ, NewLocation.Z);
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		if (!IsWalkable(Hit))
		{
			...
			if (bNormalTowardsMe)
			{
				//UE_LOG(LogBaseCharacterMovement, VeryVerbose, TEXT("- Reject StepUp (unwalkable normal %s opposed to movement)"), *Hit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
			...
			{
				//UE_LOG(LogBaseCharacterMovement, VeryVerbose, TEXT("- Reject StepUp (unwalkable normal %s above old position)"), *Hit.ImpactNormal.ToString());
				ScopedStepUpMovement.RevertMove();
				return false;
			}
		}
		...
		if (!IsWithinEdgeTolerance(Hit.Location, Hit.ImpactPoint, PawnRadius))
		{
			//UE_LOG(LogBaseCharacterMovement, VeryVerbose, TEXT("- Reject StepUp (outside edge tolerance)"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
		...
		{
			//UE_LOG(LogBaseCharacterMovement, VeryVerbose, TEXT("- Reject StepUp (up onto surface with !CanStepUp())"));
			ScopedStepUpMovement.RevertMove();
			return false;
		}
...
void UBaseCharacterMovementComponent::HandleImpact(const FHitResult& Impact, float TimeSlice, const FVector& MoveDelta)
{
...
void UBaseCharacterMovementComponent::ApplyImpactPhysicsForces(const FHitResult& Impact, const FVector& ImpactAcceleration, const FVector& ImpactVelocity)
{
	if (bEnablePhysicsInteraction && Impact.bBlockingHit )
	{
		if (UPrimitiveComponent* ImpactComponent = Impact.GetComponent())
		{
			...
			else if (BaseCharacterMovementCVars::bGeometryCollectionImpulseWorkAround)
			{
...
FString UBaseCharacterMovementComponent::GetMovementName() const
{
	if( CharacterOwner )
	{
		if ( CharacterOwner->GetRootComponent() && CharacterOwner->GetRootComponent()->IsSimulatingPhysics() )
...
void UBaseCharacterMovementComponent::DisplayDebug(UCanvas* Canvas, const FDebugDisplayInfo& DebugDisplay, float& YL, float& YPos)
{
...
void UBaseCharacterMovementComponent::ForceReplicationUpdate()
{
...
void UBaseCharacterMovementComponent::ForceClientAdjustment()
{
	ServerLastClientAdjustmentTime = -1.f;
}

FVector UBaseCharacterMovementComponent::ConstrainInputAcceleration(const FVector& InputAcceleration) const
{
...
FVector UBaseCharacterMovementComponent::ScaleInputAcceleration(const FVector& InputAcceleration) const
{
	return GetMaxAcceleration() * InputAcceleration.GetClampedToMaxSize(1.0f);
}


FVector UBaseCharacterMovementComponent::RoundAcceleration(FVector InAccel) const
{
...
float UBaseCharacterMovementComponent::ComputeAnalogInputModifier() const
{
	const float MaxAccel = GetMaxAcceleration();
...
float UBaseCharacterMovementComponent::GetAnalogInputModifier() const
{
	return AnalogInputModifier;
}

float UBaseCharacterMovementComponent::GetSimulationTimeStep(float RemainingTime, int32 Iterations) const
{
	...
	if (RemainingTime > MaxSimulationTimeStep)
	{
		...
		else
		{
			...
			{
				UE_LOG(LogBaseCharacterMovement, Warning, TEXT("GetSimulationTimeStep() - Max iterations %d hit while remaining time %.6f > MaxSimulationTimeStep (%.3f) for '%s', movement '%s'"), MaxSimulationIterations, RemainingTime, MaxSimulationTimeStep, *GetNameSafe(CharacterOwner), *GetMovementName());
			}
#endif
		}
	}
...
void UBaseCharacterMovementComponent::SmoothCorrection(const FVector& OldLocation, const FQuat& OldRotation, const FVector& NewLocation, const FQuat& NewRotation)
{
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
		if ( BaseCharacterMovementCVars::NetVisualizeSimulatedCorrections >= 2 )
		{
...
void UBaseCharacterMovementComponent::SmoothClientPosition(float DeltaSeconds)
{
...
void UBaseCharacterMovementComponent::SmoothClientPosition_Interpolate(float DeltaSeconds)
{
	...
	if (ClientData)
	{
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
			// Show lerp percent
			if ( BaseCharacterMovementCVars::NetVisualizeSimulatedCorrections >= 1 )
			{
				const FColor DebugColor = FColor::White;
		...
		if ( BaseCharacterMovementCVars::NetVisualizeSimulatedCorrections >= 1 )
		{
			...
			if ( BaseCharacterMovementCVars::NetVisualizeSimulatedCorrections >= 3 )
			{
...
void UBaseCharacterMovementComponent::SmoothClientPosition_UpdateVisuals()
{
...
bool UBaseCharacterMovementComponent::ClientUpdatePositionAfterServerUpdate()
{
	...
	const FBaseRootMotionSourceGroup BackupRootMotion = CurrentRootMotion;
	...
	for (int32 i=0; i<ClientData->SavedMoves.Num(); i++)
	{
		...
		if (ShouldUsePackedMovementRPCs())
		{
			...
			if (FBaseCharacterNetworkMoveData* NewMove = GetNetworkMoveDataContainer().GetNewMoveData())
			{
				SetCurrentNetworkMoveData(NewMove);
				NewMove->ClientFillNetworkMoveData(*CurrentMove, FBaseCharacterNetworkMoveData::ENetworkMoveType::NewMove);
			}
		}
...
bool UBaseCharacterMovementComponent::ForcePositionUpdate(float DeltaTime)
{
...
FNetworkPredictionData_Server* UBaseCharacterMovementComponent::GetPredictionData_Server() const
{
...
FNetworkPredictionData_Client_Character* UBaseCharacterMovementComponent::GetPredictionData_Client_Character() const
{
...
FNetworkPredictionData_Server_Character* UBaseCharacterMovementComponent::GetPredictionData_Server_Character() const
{
...
bool UBaseCharacterMovementComponent::HasPredictionData_Client() const
{
	return (ClientPredictionData != nullptr) && HasValidData();
}

bool UBaseCharacterMovementComponent::HasPredictionData_Server() const
{
	return (ServerPredictionData != nullptr) && HasValidData();
}

void UBaseCharacterMovementComponent::ResetPredictionData_Client()
{
...
void UBaseCharacterMovementComponent::ResetPredictionData_Server()
{
	ForceClientAdjustment();
	if (ServerPredictionData)
	{
...
float FNetworkPredictionData_Client_Character::UpdateTimeStampAndDeltaTime(float DeltaTime, class ABaseCharacter & CharacterOwner, class UBaseCharacterMovementComponent & CharacterMovementComponent)
{
...
void UBaseCharacterMovementComponent::ReplicateMoveToServer(float DeltaTime, const FVector& NewAcceleration)
{
	...
	if (const FSavedMove_Character* PendingMove = ClientData->PendingMove.Get())
	{
		if (PendingMove->CanCombineWith(NewMovePtr, CharacterOwner, ClientData->MaxMoveDeltaTime * CharacterOwner->GetActorTimeDilation(*MyWorld)))
		{
			...
			if (bAttachedToObject || !OverlapTest(OldStartLocation, PendingMove->StartRotation.Quaternion(), UpdatedComponent->GetCollisionObjectType(), GetPawnCapsuleCollisionShape(CGSHRINK_None), CharacterOwner))
			{
	...
	if (CharacterOwner->IsReplicatingMovement())
	{
		...
		const bool bCanDelayMove = (BaseCharacterMovementCVars::NetEnableMoveCombining != 0) && CanDelaySendingMove(NewMovePtr);
		...
		if (ClientData->DebugForcedPacketLossTimerStart > 0.f && (TimeSinceLossStart < BaseCharacterMovementCVars::NetForceClientServerMoveLossDuration))
		{
			bSendServerMove = false;
			UE_LOG(LogNetPlayerMovement, Log, TEXT("Drop ServerMove, %.2f time remains"), BaseCharacterMovementCVars::NetForceClientServerMoveLossDuration - TimeSinceLossStart);
		}
		else if (BaseCharacterMovementCVars::NetForceClientServerMoveLossPercent != 0.f && (RandomStream.FRand() < BaseCharacterMovementCVars::NetForceClientServerMoveLossPercent))
		{
			bSendServerMove = false;
			ClientData->DebugForcedPacketLossTimerStart = (BaseCharacterMovementCVars::NetForceClientServerMoveLossDuration > 0) ? MyWorld->RealTimeSeconds : 0.0f;
			UE_LOG(LogNetPlayerMovement, Log, TEXT("Drop ServerMove, %.2f time remains"), BaseCharacterMovementCVars::NetForceClientServerMoveLossDuration);
		}
		else
		{
...
void UBaseCharacterMovementComponent::CallServerMovePacked(const FSavedMove_Character* NewMove, const FSavedMove_Character* PendingMove, const FSavedMove_Character* OldMove)
{
	// Get storage container we'll be using and fill it with movement data
	FBaseCharacterNetworkMoveDataContainer& MoveDataContainer = GetNetworkMoveDataContainer();
	...
	static FBaseCharacterServerMovePackedBits PackedBits;
...
void UBaseCharacterMovementComponent::CallServerMove
	(
...
void UBaseCharacterMovementComponent::ServerMoveOld_Implementation
	(
	float OldTimeStamp,
...
void UBaseCharacterMovementComponent::ServerMoveDual_Implementation(
	float TimeStamp0,
...
void UBaseCharacterMovementComponent::ServerMoveDualHybridRootMotion_Implementation(
	float TimeStamp0,
	FVector_NetQuantize10 InAccel0,
	uint8 PendingFlags,
	uint32 View0,
...
bool UBaseCharacterMovementComponent::VerifyClientTimeStamp(float TimeStamp, FNetworkPredictionData_Server_Character& ServerData)
{
...
void UBaseCharacterMovementComponent::ProcessClientTimeStampForTimeDiscrepancy(float ClientTimeStamp, FNetworkPredictionData_Server_Character& ServerData)
{
	// Should only be called on server in network games
	check(CharacterOwner != NULL);
	check(CharacterOwner->GetLocalRole() == ROLE_Authority);
	...
	if (GameNetworkManager != nullptr && GameNetworkManager->bMovementTimeDiscrepancyDetection && bServerMoveHasOccurred)
	{
		...
		if (BaseCharacterMovementCVars::DebugTimeDiscrepancy > 0)
		{
		...
		if (ServerData.bResolvingTimeDiscrepancy)
		{
			...
			DeltaTimeAfterPayback = FMath::Max(DeltaTimeAfterPayback, UBaseCharacterMovementComponent::MIN_TICK_TIME);
			...
			if (BaseCharacterMovementCVars::DebugTimeDiscrepancy > 1)
			{
...
bool UBaseCharacterMovementComponent::IsClientTimeStampValid(float TimeStamp, const FNetworkPredictionData_Server_Character& ServerData, bool& bTimeStampResetDetected) const
{
	...
	if (DeltaTimeStamp < UBaseCharacterMovementComponent::MIN_TICK_TIME)
	{
		return false;
	}
...
void UBaseCharacterMovementComponent::OnClientTimeStampResetDetected()
{
}

void UBaseCharacterMovementComponent::OnTimeDiscrepancyDetected(float CurrentTimeDiscrepancy, float LifetimeRawTimeDiscrepancy, float Lifetime, float CurrentMoveError)
{
...
bool FBaseCharacterNetworkSerializationPackedBits::NetSerialize(FArchive& Ar, class UPackageMap* Map, bool& bOutSuccess)
{
	...
	if (NumBits > static_cast<uint32>(BaseCharacterMovementCVars::NetPackedMovementMaxBits))
	{
		// Protect against bad data that could cause server to allocate way too much memory.
		UE_LOG(LogNetPlayerMovement, Error, TEXT("FBaseCharacterNetworkSerializationPackedBits::NetSerialize: Dropping move due to NumBits (%d) exceeding allowable limit (%d). See NetPackedMovementMaxBits."), NumBits, BaseCharacterMovementCVars::NetPackedMovementMaxBits);
		return false;
	}
...
void FBaseCharacterNetworkMoveDataContainer::ClientFillNetworkMoveData(const FSavedMove_Character* ClientNewMove, const FSavedMove_Character* ClientPendingMove, const FSavedMove_Character* ClientOldMove)
{
	bDisableCombinedScopedMove = false;

	if (ensure(ClientNewMove))
	{
		NewMoveData->ClientFillNetworkMoveData(*ClientNewMove, FBaseCharacterNetworkMoveData::ENetworkMoveType::NewMove);
	...
	if (bHasPendingMove)
	{
		PendingMoveData->ClientFillNetworkMoveData(*ClientPendingMove, FBaseCharacterNetworkMoveData::ENetworkMoveType::PendingMove);
	...
	if (bHasOldMove)
	{
		OldMoveData->ClientFillNetworkMoveData(*ClientOldMove, FBaseCharacterNetworkMoveData::ENetworkMoveType::OldMove);
	}
}


bool FBaseCharacterNetworkMoveDataContainer::Serialize(UBaseCharacterMovementComponent& CharacterMovement, FArchive& Ar, UPackageMap* PackageMap)
{
	...
	if (!NewMoveData->Serialize(CharacterMovement, Ar, PackageMap, FBaseCharacterNetworkMoveData::ENetworkMoveType::NewMove))
	{
		return false;
	}
	...
	if (bHasPendingMove)
	{
		...
		if (!PendingMoveData->Serialize(CharacterMovement, Ar, PackageMap, FBaseCharacterNetworkMoveData::ENetworkMoveType::PendingMove))
		{
			return false;
		}
	}
	...
	if (bHasOldMove)
	{
		if (!OldMoveData->Serialize(CharacterMovement, Ar, PackageMap, FBaseCharacterNetworkMoveData::ENetworkMoveType::OldMove))
		{
			return false;
		}
	}
...
void FBaseCharacterNetworkMoveData::ClientFillNetworkMoveData(const FSavedMove_Character& ClientMove, FBaseCharacterNetworkMoveData::ENetworkMoveType MoveType)
{
	...
	const bool bSendBaseRelativeAcceleration = BaseCharacterMovementCVars::NetUseBaseRelativeAcceleration && bSendBaseRelativeLocation;
...
bool FBaseCharacterNetworkMoveData::Serialize(UBaseCharacterMovementComponent& CharacterMovement, FArchive& Ar, UPackageMap* PackageMap, FBaseCharacterNetworkMoveData::ENetworkMoveType MoveType)
{
	NetworkMoveType = MoveType;
...
void UBaseCharacterMovementComponent::ServerMovePacked_ClientSend(const FBaseCharacterServerMovePackedBits& PackedBits)
{
...
void UBaseCharacterMovementComponent::ServerMovePacked_ServerReceive(const FBaseCharacterServerMovePackedBits& PackedBits)
{
	if (!HasValidData() || !IsActive())
	{
		return;
	...
	if (NumBits > BaseCharacterMovementCVars::NetPackedMovementMaxBits)
	{
		// Protect against bad data that could cause server to allocate way too much memory.
		UE_LOG(LogNetPlayerMovement, Error, TEXT("ServerMovePacked_ServerReceive (%s): Dropping move due to NumBits (%d) exceeding allowable limit (%d). See NetPackedMovementMaxBits."), *GetNameSafe(GetOwner()), NumBits, BaseCharacterMovementCVars::NetPackedMovementMaxBits);
		return;
	}
...
void UBaseCharacterMovementComponent::ServerMove_HandleMoveData(const FBaseCharacterNetworkMoveDataContainer& MoveDataContainer)
{
	// Optional "old move"
	if (MoveDataContainer.bHasOldMove)
	{
		if (FBaseCharacterNetworkMoveData* OldMove = MoveDataContainer.GetOldMoveData())
		{
	...
	{
		if (FBaseCharacterNetworkMoveData* PendingMove = MoveDataContainer.GetPendingMoveData())
		{
	...
	// Final standard move
	if (FBaseCharacterNetworkMoveData* NewMove = MoveDataContainer.GetNewMoveData())
	{
		SetCurrentNetworkMoveData(NewMove);
...
void UBaseCharacterMovementComponent::ServerMove_PerformMovement(const FBaseCharacterNetworkMoveData& MoveData)
{
	// SCOPE_CYCLE_COUNTER(STAT_CharacterMovementServerMove);
	// CSV_SCOPED_TIMING_STAT(CharacterMovement, CharacterMovementServerMove);
	...
	if (BaseCharacterMovementCVars::NetUseBaseRelativeAcceleration && MovementBaseUtility::IsDynamicBase(MoveData.MovementBase))
	{
	...
	if( !VerifyClientTimeStamp(ClientTimeStamp, *ServerData) )
	{
		...
		if (ServerTimeStamp > 1.0f && FMath::Abs(ServerTimeStamp - ClientTimeStamp) > BaseCharacterMovementCVars::NetServerMoveTimestampExpiredWarningThreshold)
		{
	...
	if (MoveData.NetworkMoveType == FBaseCharacterNetworkMoveData::ENetworkMoveType::NewMove)
	{
...
void UBaseCharacterMovementComponent::ServerMove(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 CompressedMoveFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
	if (MovementBaseUtility::IsDynamicBase(ClientMovementBase))
	{
		//UE_LOG(LogBaseCharacterMovement, Log, TEXT("ServerMove: base %s"), *ClientMovementBase->GetName());
	...
	{
		//UE_LOG(LogBaseCharacterMovement, Log, TEXT("ServerMoveNoBase"));
		CharacterOwner->ServerMoveNoBase(TimeStamp, InAccel, ClientLoc, CompressedMoveFlags, ClientRoll, View, ClientMovementMode);
	}
}

void UBaseCharacterMovementComponent::ServerMove_Implementation(
	float TimeStamp,
	FVector_NetQuantize10 InAccel,
	FVector_NetQuantize100 ClientLoc,
	uint8 MoveFlags,
	...
	uint8 ClientMovementMode)
{
	...
	if( !VerifyClientTimeStamp(TimeStamp, *ServerData) )
	{
		const float ServerTimeStamp = ServerData->CurrentClientTimeStamp;
		// This is more severe if the timestamp has a large discrepancy and hasn't been recently reset.
		if (ServerTimeStamp > 1.0f && FMath::Abs(ServerTimeStamp - TimeStamp) > BaseCharacterMovementCVars::NetServerMoveTimestampExpiredWarningThreshold)
		{
...
void UBaseCharacterMovementComponent::ServerMoveHandleClientError(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& RelativeClientLoc, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
	if (!ShouldUsePackedMovementRPCs())
	{
	...
	const float ClientAuthorityThreshold = BaseCharacterMovementCVars::ClientAuthorityThresholdOnBaseChange;
	const float MaxFallingCorrectionLeash = BaseCharacterMovementCVars::MaxFallingCorrectionLeash;
	...
	if (bDeferServerCorrectionsWhenFalling)
	{
		...
		if (MaxServerClientErrorWhileFalling > 0.f && (bServerIsFalling || bClientIsFalling))
		{
			...
			else
			{
				...
				ServerLoc = ServerLoc - LocDiff.GetSafeNormal() * FMath::Clamp(MaxServerClientErrorWhileFalling - BaseCharacterMovementCVars::MaxFallingCorrectionLeashBuffer, 0.f, MaxServerClientErrorWhileFalling);
			}
		}
	}
	...
	if (ServerData->bForceClientUpdate || (!bFallingWithinAcceptableError && ServerCheckClientError(ClientTimeStamp, DeltaTime, Accel, ClientLoc, RelativeClientLoc, ClientMovementBase, ClientBaseBoneName, ClientMovementMode)))
	{
		...
		if (ServerData->PendingAdjustment.bBaseRelativePosition)
		{
			...
			else
			{
				...
				if (BaseCharacterMovementCVars::NetUseBaseRelativeVelocity)
				{
...
#if !UE_BUILD_SHIPPING
		if (BaseCharacterMovementCVars::NetShowCorrections != 0)
		{
			...
			const float DebugLifetime = BaseCharacterMovementCVars::NetCorrectionLifetime;
...
bool UBaseCharacterMovementComponent::ServerCheckClientError(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& ClientWorldLocation, const FVector& RelativeClientLocation, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
...
#if !(UE_BUILD_SHIPPING || UE_BUILD_TEST)
		if (BaseCharacterMovementCVars::NetForceClientAdjustmentPercent > UE_SMALL_NUMBER)
		{
			if (RandomStream.FRand() < BaseCharacterMovementCVars::NetForceClientAdjustmentPercent)
			{
...
bool UBaseCharacterMovementComponent::ServerExceedsAllowablePositionError(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& ClientWorldLocation, const FVector& RelativeClientLocation, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
...
bool UBaseCharacterMovementComponent::ServerShouldUseAuthoritativePosition(float ClientTimeStamp, float DeltaTime, const FVector& Accel, const FVector& ClientWorldLocation, const FVector& RelativeClientLocation, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
...
bool UBaseCharacterMovementComponent::ServerMove_Validate(float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 MoveFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
	return true;
}

void UBaseCharacterMovementComponent::ServerMoveDual(float TimeStamp0, FVector_NetQuantize10 InAccel0, uint8 PendingFlags, uint32 View0, float TimeStamp, FVector_NetQuantize10 InAccel, FVector_NetQuantize100 ClientLoc, uint8 NewFlags, uint8 ClientRoll, uint32 View, UPrimitiveComponent* ClientMovementBase, FName ClientBaseBoneName, uint8 ClientMovementMode)
{
	if (MovementBaseUtility::IsDynamicBase(ClientMovementBase))
	{
		//UE_LOG(LogBaseCharacterMovement, Log, TEXT("ServerMoveDual: base %s"), *ClientMovementBase->GetName());
	...
	{
		//UE_LOG(LogBaseCharacterMovement, Log, TEXT("ServerMoveDualNoBase"));
...
void UBaseCharacterMovementComponent::ServerMoveOld(float OldTimeStamp, FVector_NetQuantize10 OldAccel, uint8 OldMoveFlags)
{
	CharacterOwner->ServerMoveOld(OldTimeStamp, OldAccel, OldMoveFlags);
}

bool UBaseCharacterMovementComponent::ServerMoveOld_Validate(float OldTimeStamp, FVector_NetQuantize10 OldAccel, uint8 OldMoveFlags)
{
	return true;
}
...
void UBaseCharacterMovementComponent::MoveAutonomous
	(
	...
	)
{
	...
	if( CharacterOwner && !CharacterOwner->bClientUpdating && !CharacterOwner->IsPlayingRootMotion() && CharacterOwner->GetMesh() )
	{
		...
		if (!BaseCharacterMovementCVars::EnableQueuedAnimEventsOnServer || CharacterOwner->GetMesh()->ShouldOnlyTickMontages(DeltaTime))
		{
	...
	if (CharacterOwner && UpdatedComponent)
	{
		...
		if (BaseCharacterMovementCVars::NetEnableListenServerSmoothing &&
			CharacterOwner->GetRemoteRole() == ROLE_AutonomousProxy &&
...
void UBaseCharacterMovementComponent::UpdateFloorFromAdjustment()
{
	if (!HasValidData())
	{
		return;
...
void UBaseCharacterMovementComponent::MoveResponsePacked_ServerSend(const FBaseCharacterMoveResponsePackedBits& PackedBits)
{
...
void UBaseCharacterMovementComponent::MoveResponsePacked_ClientReceive(const FBaseCharacterMoveResponsePackedBits& PackedBits)
{
	if (!HasValidData() || !IsActive())
	{
		return;
	...
	const int32 NumBits = PackedBits.DataBits.Num();
	if (!ensureMsgf(NumBits <= BaseCharacterMovementCVars::NetPackedMovementMaxBits, TEXT("MoveResponsePacked_ClientReceive: NumBits (%d) exceeds BaseCharacterMovementCVars::NetPackedMovementMaxBits (%d)"), NumBits, BaseCharacterMovementCVars::NetPackedMovementMaxBits))
	{
	...
	// We had to wait until now and use the temp bit stream because the RPC doesn't know about the virtual overrides on the possibly custom struct that is our data container.
	FBaseCharacterMoveResponseDataContainer& ResponseDataContainer = GetMoveResponseDataContainer();
...
void UBaseCharacterMovementComponent::ServerSendMoveResponse(const FBaseClientAdjustment& PendingAdjustment)
{
	// Get storage container we'll be using and fill it with movement data
	FBaseCharacterMoveResponseDataContainer& ResponseDataContainer = GetMoveResponseDataContainer();
	...
	// 'static' to avoid reallocation each invocation
	static FBaseCharacterMoveResponsePackedBits PackedBits;
	UNetConnection* NetConnection = CharacterOwner->GetNetConnection();	
...
void FBaseCharacterMoveResponseDataContainer::ServerFillResponseData(const UBaseCharacterMovementComponent& CharacterMovement, const FBaseClientAdjustment& PendingAdjustment)
{
	bHasBase = false;
	bHasRotation = false;
	...
	if (!PendingAdjustment.bAckGoodMove)
	{
		...
		if (const ABaseCharacter* CharacterOwner = CharacterMovement.GetCharacterOwner())
		{
...
bool FBaseCharacterMoveResponseDataContainer::Serialize(UBaseCharacterMovementComponent& CharacterMovement, FArchive& Ar, UPackageMap* PackageMap)
{
	bool bLocalSuccess = true;
	const bool bIsSaving = Ar.IsSaving();
	...
	if (IsCorrection())
	{
		...
		NetSerializeOptionalValue(bIsSaving, Ar, ClientAdjustment.GravityDirection, UBaseCharacterMovementComponent::DefaultGravityDirection, PackageMap);
		...
		{
			if (FBaseRootMotionSourceGroup* RootMotionSourceGroup = GetRootMotionSourceGroup(CharacterMovement))
			{
...
void UBaseCharacterMovementComponent::ClientHandleMoveResponse(const FBaseCharacterMoveResponseDataContainer& MoveResponse)
{
	...
	else
	{
		...
		{
			if (FBaseRootMotionSourceGroup* RootMotionSourceGroup = MoveResponse.GetRootMotionSourceGroup(*this))
			{
...
FBaseRootMotionSourceGroup* FBaseCharacterMoveResponseDataContainer::GetRootMotionSourceGroup(UBaseCharacterMovementComponent& CharacterMovement) const
{
	if (!bRootMotionSourceCorrection)
	{
		return nullptr;
...
void UBaseCharacterMovementComponent::SendClientAdjustment()
{
	if (!HasValidData())
	{
		return;
...
void UBaseCharacterMovementComponent::ClientVeryShortAdjustPosition(float TimeStamp, FVector NewLoc, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
...
void UBaseCharacterMovementComponent::ClientVeryShortAdjustPosition_Implementation
	(
	float TimeStamp,
	FVector NewLoc,
...
void UBaseCharacterMovementComponent::ClientAdjustPosition(float TimeStamp, FVector NewLoc, FVector NewVel, UPrimitiveComponent* NewBase, FName NewBaseBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
...
void UBaseCharacterMovementComponent::ClientAdjustPosition_Implementation
	(
	float TimeStamp,
	FVector NewLocation,
	FVector NewVelocity,
	...
	)
{
	...
	const FBaseCharacterMoveResponseDataContainer& ResponseDataContainer = GetMoveResponseDataContainer();
	...
	if (BaseCharacterMovementCVars::bUseLastGoodRotationDuringCorrection
...
void UBaseCharacterMovementComponent::ClientAdjustRootMotionPosition_Implementation(
	float TimeStamp,
...
void UBaseCharacterMovementComponent::ClientAdjustRootMotionSourcePosition(float TimeStamp, FBaseRootMotionSourceGroup ServerRootMotion, bool bHasAnimRootMotion, float ServerMontageTrackPosition, FVector ServerLoc, FVector_NetQuantizeNormal ServerRotation, float ServerVelZ, UPrimitiveComponent* ServerBase, FName ServerBoneName, bool bHasBase, bool bBaseRelativePosition, uint8 ServerMovementMode)
{
...
void UBaseCharacterMovementComponent::ClientAdjustRootMotionSourcePosition_Implementation(
	float TimeStamp,
	FBaseRootMotionSourceGroup ServerRootMotion,
...
void UBaseCharacterMovementComponent::ClientAckGoodMove(float TimeStamp)
{
	CharacterOwner->ClientAckGoodMove(TimeStamp);
}

void UBaseCharacterMovementComponent::ClientAckGoodMove_Implementation(float TimeStamp)
{
	if (!HasValidData() || !IsActive())
	{
		return;
...
void UBaseCharacterMovementComponent::CapsuleTouched(UPrimitiveComponent* OverlappedComp, AActor* Other, UPrimitiveComponent* OtherComp, int32 OtherBodyIndex, bool bFromSweep, const FHitResult& SweepResult )
{
	if (!bEnablePhysicsInteraction)
	{
		return;
...
FVector UBaseCharacterMovementComponent::GetLastUpdateRequestedVelocity() const
{
	return LastUpdateRequestedVelocity;
}

void UBaseCharacterMovementComponent::SetAvoidanceGroup(int32 GroupFlags)
{
	SetAvoidanceGroupMask(GroupFlags);
}

void UBaseCharacterMovementComponent::SetAvoidanceGroupMask(const FNavAvoidanceMask& GroupMask)
{
	SetAvoidanceGroupMask(GroupMask.Packed);
}

void UBaseCharacterMovementComponent::SetGroupsToAvoid(int32 GroupFlags)
{
	SetGroupsToAvoidMask(GroupFlags);
}

void UBaseCharacterMovementComponent::SetGroupsToAvoidMask(const FNavAvoidanceMask& GroupMask)
{
	SetGroupsToAvoidMask(GroupMask.Packed);
}

void UBaseCharacterMovementComponent::SetGroupsToIgnore(int32 GroupFlags)
{
	SetGroupsToIgnoreMask(GroupFlags);
}

void UBaseCharacterMovementComponent::SetGroupsToIgnoreMask(const FNavAvoidanceMask& GroupMask)
{
	SetGroupsToIgnoreMask(GroupMask.Packed);
}

void UBaseCharacterMovementComponent::SetAvoidanceEnabled(bool bEnable)
{
...
void UBaseCharacterMovementComponent::ApplyDownwardForce(float DeltaSeconds)
{
...
void UBaseCharacterMovementComponent::ApplyRepulsionForce(float DeltaSeconds)
{
	if (UpdatedPrimitive && RepulsionForce > 0.0f && CharacterOwner!=nullptr)
	{
		...
		if (Overlaps.Num() > 0)
		{
			...
			for (int32 i=0; i < Overlaps.Num(); i++)
			{
				...
				if (!OverlapBody)
				{
					UE_LOG(LogBaseCharacterMovement, Warning, TEXT("%s could not find overlap body for body index %d"), *GetName(), OverlapBodyIndex);
					continue;
				}
...
void UBaseCharacterMovementComponent::ApplyAccumulatedForces(float DeltaSeconds)
{
...
void UBaseCharacterMovementComponent::ClearAccumulatedForces()
{
	PendingImpulseToApply = FVector::ZeroVector;
	PendingForceToApply = FVector::ZeroVector;
	PendingLaunchVelocity = FVector::ZeroVector;
...
void UBaseCharacterMovementComponent::AddRadialForce(const FVector& Origin, float Radius, float Strength, enum ERadialImpulseFalloff Falloff)
{
...
void UBaseCharacterMovementComponent::AddRadialImpulse(const FVector& Origin, float Radius, float Strength, enum ERadialImpulseFalloff Falloff, bool bVelChange)
{
	FVector Delta = UpdatedComponent->GetComponentLocation() - Origin;
	const float DeltaMagnitude = Delta.Size();
...
void UBaseCharacterMovementComponent::RegisterComponentTickFunctions(bool bRegister)
{
...
void UBaseCharacterMovementComponent::ApplyWorldOffset(const FVector& InOffset, bool bWorldShift)
{
...
void UBaseCharacterMovementComponent::TickCharacterPose(float DeltaTime)
{
	if (DeltaTime < UBaseCharacterMovementComponent::MIN_TICK_TIME)
	{
		return;
	}
	...
	if (CharacterMesh->ShouldTickPose())
	{
		...
		if (CharacterOwner->IsPlayingRootMotion() || bWasPlayingRootMotion)
		{
			...
			{
				FAnimMontageInstance* RootMotionMontageInstance = CharacterOwner->GetRootMotionAnimMontageInstance();
				UE_LOG(LogRootMotion, Log, TEXT("UBaseCharacterMovementComponent::TickCharacterPose Role: %s, RootMotionMontage: %s, MontagePos: %f, DeltaTime: %f, ExtractedRootMotion: %s, AccumulatedRootMotion: %s")
...
bool UBaseCharacterMovementComponent::HasRootMotionSources() const
{
...
uint16 UBaseCharacterMovementComponent::ApplyRootMotionSource(TSharedPtr<FBaseRootMotionSource> SourcePtr)
{
...
void UBaseCharacterMovementComponent::OnRootMotionSourceBeingApplied(const FBaseRootMotionSource* Source)
{
}

TSharedPtr<FBaseRootMotionSource> UBaseCharacterMovementComponent::GetRootMotionSource(FName InstanceName)
{
	return CurrentRootMotion.GetRootMotionSource(InstanceName);
}

TSharedPtr<FBaseRootMotionSource> UBaseCharacterMovementComponent::GetRootMotionSourceByID(uint16 RootMotionSourceID)
{
	return CurrentRootMotion.GetRootMotionSourceByID(RootMotionSourceID);
}

void UBaseCharacterMovementComponent::RemoveRootMotionSource(FName InstanceName)
{
	CurrentRootMotion.RemoveRootMotionSource(InstanceName);
}

void UBaseCharacterMovementComponent::RemoveRootMotionSourceByID(uint16 RootMotionSourceID)
{
	CurrentRootMotion.RemoveRootMotionSourceByID(RootMotionSourceID);
}

void UBaseCharacterMovementComponent::ConvertRootMotionServerIDsToLocalIDs(const FBaseRootMotionSourceGroup& LocalRootMotionToMatchWith, FBaseRootMotionSourceGroup& InOutServerRootMotion, float TimeStamp)
{
	...
	for (int32 MappingIndex = RootMotionIDMappings.Num()-1; MappingIndex>=0; MappingIndex--)
	{
		...
		for (const TSharedPtr<FBaseRootMotionSource>& LocalRootMotionSource : LocalRootMotionToMatchWith.RootMotionSources)
		{
	...
	// FBaseRootMotionSource::LocalID is an ID added when that Source is applied.
	// When we receive RootMotionSource data from the server, LocalIDs on that
	// RootMotion data are the server LocalIDs. When processing an FBaseRootMotionSourceGroup
	...
	for (TSharedPtr<FBaseRootMotionSource>& ServerRootMotionSource : InOutServerRootMotion.RootMotionSources)
	{
		if (ServerRootMotionSource.IsValid())
		{
			...
			ServerRootMotionSource->LocalID = (uint16)EBaseRootMotionSourceID::Invalid;
			...
			// If we do, change it to that mapping and update the timestamp
			{
				...
				for (FBaseRootMotionServerToLocalIDMapping& Mapping : RootMotionIDMappings)
				{
			...
			TArray<TSharedPtr<FBaseRootMotionSource>> LocalRootMotionSources;
			...
			for (const TSharedPtr<FBaseRootMotionSource>& LocalRootMotionSource : LocalRootMotionSources)
			{
				if (LocalRootMotionSource.IsValid())
				{
					...
					if (LocalRootMotionSource->Matches(ServerRootMotionSource.Get()))
					{
						...
						// Add to Mapping
						{
							FBaseRootMotionServerToLocalIDMapping NewMapping;
...
FNetworkPredictionData_Client_Character::FNetworkPredictionData_Client_Character(const UBaseCharacterMovementComponent& ClientMovement)
...
void FNetworkPredictionData_Client_Character::AckMove(int32 AckedMoveIndex, UBaseCharacterMovementComponent& CharacterMovementComponent) 
{
...
FNetworkPredictionData_Server_Character::FNetworkPredictionData_Server_Character(const UBaseCharacterMovementComponent& ServerMovement)
	: PendingAdjustment()
...
void FSavedMove_Character::Clear()
{
	...
	StartFloor = FBaseFindFloorResult();
...
void FSavedMove_Character::SetMoveFor(ABaseCharacter* Character, float InDeltaTime, FVector const& NewAccel, class FNetworkPredictionData_Client_Character & ClientData)
{
...
void FSavedMove_Character::SetInitialPosition(ABaseCharacter* Character)
{
...
void FSavedMove_Character::PostUpdate(ABaseCharacter* Character, FSavedMove_Character::EPostUpdateMode PostUpdateMode)
{
	// Common code for both recording and after a replay.
	{
		...
		if (SavedVelocity.SizeSquared() > WarnVelocitySqr)
		{
			...
			{
				UE_LOG(LogBaseCharacterMovement, Log, TEXT("FSavedMove_Character::PostUpdate detected very high Velocity! (%s), but with active root motion sources (could be intentional)"), *SavedVelocity.ToString());
			}
			else
			{
				UE_LOG(LogBaseCharacterMovement, Warning, TEXT("FSavedMove_Character::PostUpdate detected very high Velocity! (%s)"), *SavedVelocity.ToString());
			}
		}
#endif
		...
		if (BaseCharacterMovementCVars::NetUseBaseRelativeAcceleration && MovementBaseUtility::IsDynamicBase(MovementBase))
		{
...
bool UBaseCharacterMovementComponent::CanDelaySendingMove(const FSavedMovePtr& NewMovePtr)
{
...
float UBaseCharacterMovementComponent::GetClientNetSendDeltaTime(const APlayerController* PC, const FNetworkPredictionData_Client_Character* ClientData, const FSavedMovePtr& NewMove) const
{
...
{
	return PC ? StartControlRotation.Equals(PC->GetControlRotation(), BaseCharacterMovementCVars::NetStationaryRotationTolerance) : false;
}

void FSavedMove_Character::GetPackedAngles(uint32& YawAndPitchPack, uint8& RollPack) const
{
	// Compress rotation down to 5 bytes
	YawAndPitchPack = UBaseCharacterMovementComponent::PackYawAndPitchTo32(SavedControlRotation.Yaw, SavedControlRotation.Pitch);
	RollPack = FRotator::CompressAxisToByte(SavedControlRotation.Roll);
}

bool FSavedMove_Character::CanCombineWith(const FSavedMovePtr& NewMovePtr, ABaseCharacter* Character, float MaxDelta) const
{
	const FSavedMove_Character* NewMove = NewMovePtr.Get();
	...
	const bool bPreventOnStaticBaseChange = (BaseCharacterMovementCVars::NetEnableMoveCombiningOnStaticBaseChange == 0);
	...
	if (NewStartAttachParent != nullptr)
	{
		...
		if (!RelativeLocationDelta.IsNearlyZero(BaseCharacterMovementCVars::NetMoveCombiningAttachedLocationTolerance))
		{
			//UE_LOG(LogBaseCharacterMovement, Warning, TEXT("NoCombine: DeltaLocation(%s)"), *RelativeLocationDelta.ToString());
			return false;
		}
		...
		if (!RelativeRotationDelta.IsNearlyZero(BaseCharacterMovementCVars::NetMoveCombiningAttachedRotationTolerance))
		{
			return false;
		}
	}
...
void FSavedMove_Character::CombineWith(const FSavedMove_Character* OldMove, ABaseCharacter* InCharacter, APlayerController* PC, const FVector& OldStartLocation)
{
	UBaseCharacterMovementComponent* CharMovement = InCharacter->GetCharacterMovement();
...
void FSavedMove_Character::PrepMoveFor(ABaseCharacter* Character)
{
	...
	if( SavedRootMotion.HasActiveRootMotionSources() || Character->SavedRootMotion.HasActiveRootMotionSources() )
	{
		if( Character->bClientResimulateRootMotionSources )
		{
			...
			// Apply any corrections/state from either last played move or last received from server (in ABaseCharacter::SavedRootMotion)
...
void UBaseCharacterMovementComponent::FlushServerMoves()
{
...
ETeleportType UBaseCharacterMovementComponent::GetTeleportType() const
{ 
